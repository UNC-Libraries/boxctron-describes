...
class TestStreamUrlToTemp:
    ...
    async def test_successful_download(self):
        ...
        with patch("app.utils.file_handler.httpx.AsyncClient", return_value=mock_client):
    ...
    async def test_download_exceeds_max_size(self):
        ...
        mock_response = AsyncMock()
        mock_response.status_code = 200
        mock_response.aiter_bytes = AsyncMock(return_value=iter([large_content]))
    ...
    async def test_download_network_error(self):
        ...
        mock_client = AsyncMock()
        mock_client.__aenter__ = AsyncMock(side_effect=httpx.RequestError("Connection failed"))
        mock_client.__aexit__ = AsyncMock(return_value=None)
